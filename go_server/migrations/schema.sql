...
CREATE TABLE users_tab (
    ...
    password VARCHAR(255) NOT NULL DEFAULT '' COMMENT '密码（SHA256(MD5+salt)）',
    salt VARCHAR(32) NOT NULL DEFAULT '' COMMENT '密码盐值（16位随机字符串）',
    ...
    mtime INT NOT NULL DEFAULT 0 COMMENT '更新时间(UNIX时间戳)',
...
CREATE TABLE roles_tab (
    ...
    role_name VARCHAR(100) NOT NULL DEFAULT '' UNIQUE COMMENT '角色名称',
...
CREATE TABLE permissions_tab (
    ...
    permission_code VARCHAR(100) NOT NULL DEFAULT '' UNIQUE COMMENT '权限代码',
    permission_name VARCHAR(100) NOT NULL DEFAULT '' COMMENT '权限名称',
    permission_desc VARCHAR(500) NOT NULL DEFAULT '' COMMENT '权限描述',
...
CREATE TABLE sessions_tab (
    ...
    user_id BIGINT NOT NULL DEFAULT 0 COMMENT '用户ID',
    expires_at INT NOT NULL DEFAULT 0 COMMENT '过期时间(UNIX时间戳)',
    ctime INT NOT NULL DEFAULT 0 COMMENT '创建时间(UNIX时间戳)',
    INDEX idx_user_id (user_id),
...
CREATE TABLE password_reset_tokens_tab (
    ...
    user_id BIGINT NOT NULL DEFAULT 0 COMMENT '用户ID',
    token VARCHAR(64) NOT NULL DEFAULT '' UNIQUE COMMENT '重置token',
    expires_at INT NOT NULL DEFAULT 0 COMMENT '过期时间(UNIX时间戳)',
...
CREATE TABLE email_verification_codes_tab (
    ...
    purpose VARCHAR(50) NOT NULL DEFAULT '' COMMENT '用途: register=注册, reset=重置密码',
...
CREATE TABLE task_center_tab (
    ...
    task_id VARCHAR(50) NOT NULL DEFAULT '' UNIQUE COMMENT '任务ID',
    task_type TINYINT NOT NULL DEFAULT 0 COMMENT '任务类型: 1=文案生成, 2=图片生成',
    task_status TINYINT NOT NULL DEFAULT 0 COMMENT '任务状态: 0=待处理, 1=进行中, 2=已完成, 3=失败',
    operator VARCHAR(255) NOT NULL DEFAULT '' COMMENT '操作人邮箱',
    ctime INT NOT NULL DEFAULT 0 COMMENT '创建时间(UNIX时间戳)',
    mtime INT NOT NULL DEFAULT 0 COMMENT '更新时间(UNIX时间戳)',
...
CREATE TABLE copywriting_tasks_tab (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '自增ID',
    task_id VARCHAR(50) NOT NULL DEFAULT '' UNIQUE COMMENT '任务ID（关联task_center_tab）',
    ...
    analyze_model TINYINT NOT NULL DEFAULT 1 COMMENT '分析模型: 1=Gemini, 2=GPT, 3=DeepSeek',
    ...
    generate_model TINYINT NOT NULL DEFAULT 1 COMMENT '生成模型: 1=Gemini, 2=GPT, 3=DeepSeek',
...
CREATE TABLE tasks_tab (
    ...
    competitor_link VARCHAR(1000) NOT NULL DEFAULT '' COMMENT '竞品链接',
    copywriting_task_id VARCHAR(50) NOT NULL DEFAULT '' COMMENT '关联的文案任务ID',
    generate_model TINYINT NOT NULL DEFAULT 1 COMMENT '生成模型: 1=Gemini, 2=GPT, 3=DeepSeek',
    aspect_ratio VARCHAR(20) NOT NULL DEFAULT '1:1' COMMENT '图片宽高比',
...
-- 插入管理员用户（密码和salt待人工更新）
INSERT INTO users_tab (email, password, salt, username, user_type, user_status, ctime, mtime) 
VALUES ('admin@gmail.com', '', '', 'Administrator', 99, 1, UNIX_TIMESTAMP(), UNIX_TIMESTAMP());
